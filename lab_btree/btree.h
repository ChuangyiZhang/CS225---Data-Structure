...
size_t binanry_search(const vector<T>elements, const C& value, unsigned start, unsigned end){
    if( end - start == 0){
        ...
        if(elements[middle] > value)
            return binanry_search(elements, value, 0, middle);
        if(elements[middle] < value)
            return binanry_search(elements, value, middle+1, end);
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return binanry_search(elements, val, 0, elements.size() -1);
}
